...
LastEditTime: 2022-12-06 15:29:38
FilePath: \数字图像处理2\code\JPEG\RGB2YUV.py
...
def rgb2yuv(img, width, height):
    ...
    U = [[0 for i in range((width - 1) // 2 + 1)]
         for i in range((height - 1) // 2 + 1)]
    V = [[0 for i in range((width - 1) // 2 + 1)]
         for i in range((height - 1) // 2 + 1)]
    for i in range(height):
        ...
        for j in range(width):
            ...
            if i % 2 == 0 and j % 2 == 0:
                U[i // 2][j // 2] = (-0.168736 * R +
                                     (-0.331264) * G + 0.5 * B) + 128
            if flag:
                V[i // 2][j // 2] = (0.5 * R + (-0.418688)
                                     * G + (-0.081312) * B) + 128
        print( 'rgb2yuv ', i,'/',height)
...
def yuv2rgb(Y, U, V, width, height):
    ...
    for i in range(height):
        for j in range(width):
            ...
            img[i][j][1] = (Y[i][j] + 128) + (-0.34414) * (U[i//2]
                                                           [j//2] - 128) + (-0.71414) * (V[i//2][j//2] - 128)
