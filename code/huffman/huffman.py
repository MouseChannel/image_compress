...
def createTree(hist_dict: dict) -> HuffmanNode:
    ...
    while q.qsize() > 1:
        ...
        node = HuffmanNode(value=l_freq.value + r_freq.value,
                           left_child=l_freq, right_child=r_freq)
...
def put(path):
    ...
    print('原图灰度图大小', ori_size, 'KB  压缩后大小',
          comp_size, 'KB  压缩率', comp_rate, '%')

    plt.rcParams['font.sans-serif'] = ['SimHei']
    plt.subplot(121), plt.imshow(src_img, plt.cm.gray), plt.title(
        '原图灰度图像'), plt.axis('off')
    plt.subplot(122), plt.imshow(
        img_decode, plt.cm.gray), plt.title('解压后'), plt.axis('off')
...
# 平均编码长度为：6.044
# 编码效率为：0.996719
# 原图灰度图大小 1822.5 KB  压缩后大小 1376.9261474609375 KB  压缩率 0.24448496709962275 %

# 平均编码长度为：6.196
# 编码效率为：0.996060
# 原图灰度图大小 683.98046875 KB  压缩后大小 529.7354736328125 KB  压缩率 0.22551081816572338 %
